from listy.main_argument import identify_main_argument
...
async def test_english():
    ...
    result = await identify_main_argument(text)
    assert "krzysztof" in result.main_argument.lower()
    assert "pierogi" in result.main_argument.lower()
...
async def test_polish():
    ...
    assert any(
        indicator in result.main_argument.lower() for indicator in polish_indicators
    ), f"Expected Polish response, got: {result.main_argument}"
